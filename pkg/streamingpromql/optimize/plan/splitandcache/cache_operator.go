...
type CacheOperator struct {
	...
	key               []byte
	hashedKey         string
	cachedExtentsSize int // Approximate size, in bytes, of extents loaded from the cache that have not expired.
...
func (c *CacheOperator) computeCacheKey() []byte {
	// TODO
	return []byte("the-cache-key")
}
...
func (c *CacheOperator) fetchExistingExtents(ctx context.Context) ([]CachedExtent, error) {
	...
	if err := cacheEntry.Unmarshal(cacheHit); err != nil {
		// FIXME: returning an error here is simplest, but we may want to log it and continue as if there was no cached result
		return nil, err
	}
	...
	if !bytes.Equal(cacheEntry.CacheKey, c.key) {
		...
		// TODO: metrics?
		spanLogger.DebugLog("msg", "ignoring cache entry as the cache key in the entry does not match the expected key, likely due to a hash collision", "hashed_key", c.hashedKey)
		return nil, nil
	}
	...
	if err := c.MemoryConsumptionTracker.IncreaseMemoryConsumption(uint64(c.cachedExtentsSize), limiter.CachedResponses); err != nil {
		return nil, err
	}
...
func (c *CacheOperator) estimateExtentsSize(extents []CachedExtent) int {
	size := 0
...
func (c *CacheOperator) writeCacheEntry(ctx context.Context, stats *types.OperatorEvaluationStats, annos annotations.Annotations) error {
	...
	desiredTimeRangeExtent := CachedExtent{
		...
		Annotations:             querierpb.EncodeAnnotations(annos, c.originalExpression),
		Stats:                   stats.Encode(),
	...
	// TODO: don't bother caching empty series (after applying freshness window)
}
...
func (c *CacheOperator) Close() {
	...
	c.MemoryConsumptionTracker.DecreaseMemoryConsumption(uint64(c.cachedExtentsSize), limiter.CachedResponses)
