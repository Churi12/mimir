...
func TestCacheOperator(t *testing.T) {
	...
	for name, testCase := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			o := newCacheOperator(cache, materializer, nil, testCase.timeRange, memoryConsumptionTracker, posrange.PositionRange{}, "the_original_expression{}", limits, log.NewNopLogger())
			o.timeNow = func() time.Time { return timeNow }
			o.getCurrentTraceID = func(context.Context) (string, bool) { return newTraceID, true }

			cacheKey := o.computeCacheKey()
...
func TestCacheOperator_DoesNotSaveCacheEntryIfSeriesMetadataNotCalled(t *testing.T) {
	...
	o := newCacheOperator(cache, materializer, nil, timeRange, memoryConsumptionTracker, posrange.PositionRange{}, "the_original_expression{}", limits, log.NewNopLogger())
...
// TODO
// Different series in different extents - test merging behaviour

// Cache key:
// - must include offset from step somehow (eg. if step is 10s but start timestamp is 08:00:02, need to differentiate this from cache entry with step 10s and start 08:00:00)
// - must include step
// - must include lookback window
// - must include delayed name removal
// - must include original expression (so that annotation positions are correct)
